# Trip Requirements Summary

## Core Trip Requirements

- Group size: 6 people, traveling as 3 couples
- Timing: must be in June
- Trip length: roughly 10 to 12 shared days
- Origin: flying from San Francisco

## General Trip Shape

- We've agreed that the trip should start on June 9th or so. 
- Phil & Lillian really want to visit Lofoten, and they need to be back in the US by June 25th or so, so we're planning to split the group around June 18th or so: P&L will go to Lofoten, the rest migth continue traveling around Norway or potentially visit other scandinavian countries.


## Couple-by-Couple Preferences

### Christian & Eliza

- Christian likes:
  - hiking
  - sauna and athletic activities
  - beautiful landscapes and instagram worthy photos
- Eliza likes:
  - shopping
  - good dinners
  - some drinks and nightlife
  - luxury accommodations
  - animals, she visits zoos and wants to go to galapagos
- Eliza is enthusiastic about beautiful hikes in moderate weather, but is not an athlete
- Eliza is allergic to salmon and molluscs

### Phil & Lillian

- Most outdoor and adventure-oriented couple
- Like:
  - long hikes
  - bike rides
  - surfing
  - snorkeling
  - scuba diving
  - Kayaking
  - generally more intense active days and adrenaline
  - Phil loves water based activities
- They do not drink alcohol
- Important health consideration:
  - Lillian may be early in pregnancy by June, so some activities may need to remain optional
- Best fit overall: a trip with a strong active identity, not just cities plus a few token outdoor days

### Juan & Steph

- Both like being active
- Both have large tattoos that are difficult to cover in a bathing suit or when naked
- Both enjoy:
  - beautiful nature
  - cities
  - exploring towns
- Steph likes:
  - thrift shopping
  - animal viewing and interaction: she's loved safaris
  - vegetarian food access is important (or sushi is OK if that's all that's available)
  - she's a marathon runner so having access to running routes or a treadmill every day is important
- Juan likes:
  - coffee
  - people-watching
  - Beautiful landscapes
  - Saunas and relaxing after a good workout
- Juan & Steph are both planning to continue traveling around europe for another two weeks. They'd like to visit other scandiavian cities, and tag another long destination, potentially Turkey if geopolitics allows for it, or something else if not.

## What the Group Needs Overall

- A trip that balances activity, comfort, some form of night life, and nature
- A trip that has something special for everyone, since everyone has generally traveled quite a bit
- A trip where there's room for everyone to have great times together, but for everyone to also have memorable experiences alone or in couples
- A route that does not depend on constant real-time planning or "we'll figure it out when we get there"; this group is really really bad at making last minute decisions


## Likely Hard Constraints

- Must work in June. We've agreed on June 9-25 as they days when we'll be travelling though haven't bought tickets yet
- Needs good enough vegetarian/food flexibility for Stephanie and Eliza
- Should include at least some real outdoor and adventure value
- Must have backup alternatives for pregnant women that cannot use a hot tub or go scuba diving

## Likely Soft Constraints

- Some animal activities are important, but not everyone needs to join
- Shopping matters, but not enough to justify making the whole trip city-heavy
- Too many hotel changes could be an issue
- Should include one splurge stay, preferably where prices are favorable
